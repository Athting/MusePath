  ...
  return (
    <div className="video-card">
      ...
      <div className="video-info">
        ...
        <div className="video-channel page-style-231" >
          <span className="page-style-232">▶</span>
          {video.channel}
        </div>
        <div className="mt-2">
          <a
    ...
    <div className="page-content animate-fade-in">
      ...
      <div className="card card-padded mb-8">
        ...
        <div className="input-group mb-5">
          ...
          <div className="page-style-234">
            <div className="page-style-235">
              <Search size={16} className="page-style-236" />
              <input
                className="input page-style-237"
            ...
            <div className="flex flex-col gap-1">
              {INSTRUMENTS.map(i => (
                <button key={i} onClick={() => setFilters(f => ({ ...f, instrument: i, query: '' }))}
                  className={`btn btn-sm page-style-239 ${filters.instrument === i && !filters.query ? 'btn-primary' : 'btn-secondary'}`}>
            ...
            <div className="flex flex-col gap-1">
              {LEVELS.map(l => (
                <button key={l} onClick={() => setFilters(f => ({ ...f, level: l, query: '' }))}
                  className={`btn btn-sm page-style-240 ${filters.level === l && !filters.query ? 'btn-primary' : 'btn-secondary'}`}>
            ...
            <div className="flex flex-col gap-1">
              {TOPICS.map(t => (
                <button key={t} onClick={() => setFilters(f => ({ ...f, topic: t, query: '' }))}
                  className={`btn btn-sm page-style-241 ${filters.topic === t && !filters.query ? 'btn-primary' : 'btn-secondary'}`}>
        ...
        <button onClick={search} disabled={loading} className="btn btn-primary btn-lg">
          {loading ? (
            <><div className="loading-spinner page-style-242"  /> Searching...</>
          ) : (
      ...
      {loading && (
        <div className="empty-state">
          <div className="loading-spinner page-style-243"  />
          <div className="page-style-244">Searching YouTube...</div>
        </div>
      )}
      ...
      {!searched && (
        <div className="empty-state page-style-245" >
          <div className="empty-state-icon">🎬</div>
